...
def download_and_crop(
    ...
    for img_path in tqdm(sorted(detections.keys())):
        ...
        crop_path = crop_path_fun(img_path, cropped_images_dir, detector_version)
        future = pool.submit(
            load_and_crop, img_path, images_dir, container_client, bbox_dicts,
            confidence_threshold, crop_path,  #crop_path_template[is_ground_truth]
...
def crop_path_fun(image_path, cropped_images_dir, detector_version, save_each_species=True):
    temp_path = os.path.basename(image_path)
    if save_each_species:
        folders = image_path.split(os.sep)
        spec_fol = folders[-2]
        spec_dir = os.path.join(cropped_images_dir, spec_fol)
        if not os.path.exists:
            os.mkdir(spec_dir)
    else:
        spec_dir = cropped_images_dir
    final_path = os.path.join(
        spec_dir,
        temp_path + '___crop{n:>02d}_' + f'mdv{detector_version}.jpg')
    return final_path
